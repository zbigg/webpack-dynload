...
class DynamicLoadMetaPlugin {
    ...
    apply(compiler) {
        const definePlugin = new DefinePlugin({
            DYNAMIC_LOAD_DIRNAME: DefinePlugin.runtimeValue((v) => {
            ...
            compilation.hooks.afterOptimizeChunkAssets.tap("DynamicLoadMetaPlugin", (chunks) => {
                ...
                const deps = Array.from(chunks)
                    ...
                    .map((chunk) => ({
                        ...
                        defines: Array.from(chunk._modules)
                            .map(moduleLocalPath)
                            .filter(([_, moduleSrcPath]) => moduleSrcPath && moduleSrcPath.endsWith(".js")),
                ...
                Object.keys(modulesToChunkMeta).forEach((module) => {
                    ...
                    for (const [_otherId, otherModule] of chunk.requires || []) {
                        ...
                        if (!otherChunkMeta) {
                            console.log(`warning: ${chunk} don't know how to resolve ${otherModule}`);
                        } else {
                ...
                for (const key of Object.keys(modulesToChunkMeta)) {
                    modulesToChunkMeta[key] = Array.from(new Set(modulesToChunkMeta[key]));
                }
